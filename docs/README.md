...
| [`specs/2026-08-05-*.md`](specs/) | Five stretch-goal analysis specs written after Phase 3 landed (Bayesian rating inference, Monte Carlo rollouts, SPRT engine ratings, a policy-mixture engine, a Maia calibration audit), plus `2026-08-05-build-priority.md` saying which order to build them in and why. **Bayesian rating inference shipped as Task 13** — read the spec with the plan's Task 13 section next to it, since three of the spec's constants turned out to be an order of magnitude off once measured. **Monte Carlo rollouts shipped as Task 14**, same caveat: the spec's load-bearing unknown (is the ONNX batch axis dynamic?) turned out fine, but the thing it was banking on — batching being *faster* — turned out to be worth about 10%. The remaining three are specs only. |
